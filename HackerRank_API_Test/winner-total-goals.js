...
getTotalGoals('UEFA Champions League', 2011) // ans 92

async function getTotalGoals(leageName, year) {
    try {
        let team1Goal = 0, team2Goal = 0
        let winnerApiResponse = await callWinnerApi(year, leageName)
        let winnerTeamName = winnerApiResponse.data[0].winner
        let response = await Promise.all(['team1', 'team2'].map(async (teamNumber) => {
            if (teamNumber == 'team1') {
                let response = await callApi(winnerTeamName, year, teamNumber, 1)
        ...
        if (response[0] > 1) {
            ...
            await Promise.all(homeGroungPages.map(async (pageNo) => {
                let response = await callApi(winnerTeamName, year, 'team1', pageNo)
        ...
        if (response[1] > 1) {
            ...
            await Promise.all(visitingTeam.map(async (pageNo) => {
                let response = await callApi(winnerTeamName, year, 'team2', pageNo)
...
async function callWinnerApi(year, leageName) {
    return new Promise((resolve, reject) => {
        try {
            var request = require('request');
            console.log(`year  ${year}  leageName  ${leageName}  `)
            var options = {
                'method': 'GET',
                'url': `https://jsonmock.hackerrank.com/api/football_competitions?year=${year}&name=${leageName}`,
